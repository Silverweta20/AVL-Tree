...
template <class T>
bool Tree<T>::empty() {
    if(nullptr == root->getLeftNode() && nullptr == root->getRightNode()) {
        return true;
    }
    return false;
}
...
bool Tree<T>::insertNode(T& val) {
    ...
    bool resultado = insertNode(val,root);
...
void Tree<T>::rebalance(Node<T>*& node) {
    if (node == nullptr) return;
    rebalance(node->getLeftNode());
    rebalance(node->getRightNode());
    int leftDepth = depth(node->getLeftNode());
    int rightDepth = depth(node->getRightNode());
    int diff = leftDepth - rightDepth;
    if (diff > 1){
        int leftLeft  = depth(node->getLeftNode()->getLeftNode());
        int leftRight = depth(node->getLeftNode()->getRightNode());
        int diffLeft = leftLeft - leftRight;
        if (diffLeft < -1) {
        ...
        int rightRight = depth(node->getRightNode()->getRightNode());
        int rightLeft  = depth(node->getRightNode()->getLeftNode());
        int diffRight = rightRight - rightLeft;
        if (diffRight < -1) {
...
void Tree<T>::rightRotate(Node<T>*& node) {
    ...
    auxNode->setLeftNode(node->getRightNode());
    node->setRightNode(auxNode);
}
...
void Tree<T>::leftRotate(Node<T>*& node) {
    ...
    auxNode->setRightNode(node->getLeftNode());
    node->setLeftNode(auxNode);
}
